...
use crate::eval::{RuntimeEnv, Value, eval, value_debug};
use crate::syntax::{
    BinaryOp, Expr, ExprKind, FieldExpr, FieldTy, InterpPart, Ty, TyKind, UnaryOp,
};
...
pub(crate) struct Ctx {
    ...
    pub(crate) values: HashMap<String, ValueInfo>,
}
...
impl TypeChecker {
    pub(crate) fn check_well_formed_type(
        &self,
        ty: &Ty,
        ctx: &Ctx,
    ...
    pub(crate) fn synth_expr(&self, expr: &Expr, ctx: &Ctx) -> Result<(Ty, Expr), Diagnostic> {
        match &expr.kind {
    ...
    pub(crate) fn synth_builtin_app(
        &self,
        span: Span,
        name: &str,
        func_elab: Expr,
    ...
    pub(crate) fn erase_refinements(&self, ty: &Ty, ctx: &Ctx) -> Result<Ty, Diagnostic> {
        Ok(match &ty.kind {
    ...
    pub(crate) fn expand_alias(&self, ty: &Ty, ctx: &Ctx) -> Result<Ty, Diagnostic> {
        self.expand_alias_inner(ty, ctx, &mut Vec::new())
    }

    pub(crate) fn expand_alias_inner(
        &self,
        ty: &Ty,
        ctx: &Ctx,
        stack: &mut Vec<String>,
    ...
    pub(crate) fn ty_name(&self, ty: &Ty, ctx: &Ctx) -> String {
        match self
            .expand_alias(ty, ctx)
...
pub(crate) fn runtime_from_ctx(ctx: &Ctx) -> RuntimeEnv {
    ctx.values
        .iter()
...
pub(crate) fn ty_mentions_alias(ty: &Ty, name: &str) -> bool {
    match &ty.kind {
