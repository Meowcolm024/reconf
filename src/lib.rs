...
impl Span {
    fn new(file_id: usize, start: usize, end: usize) -> Self {
        Self {
            file_id,
            start,
            end,
    ...
    fn empty(file_id: usize, pos: usize) -> Self {
        Self {
            file_id,
            start: pos,
            end: pos,
...
mod tests {
    ...
    #[test]
    fn eval_positive_fixtures() {
        let cases = [
            (
                "eval_ok/simple_config.reconf",
                "{\n  \"port\": 8080,\n  \"host\": null\n}",
            ),
            ("eval_ok/interpolation.reconf", "\"Hello world!\""),
            (
                "eval_ok/module_main.reconf",
                "{\n  \"dir\": \".\",\n  \"retries\": 3\n}",
            ),
            (
                "eval_ok/list_and_methods.reconf",
                "{\n  \"has_two\": true,\n  \"count\": 3,\n  \"prefix\": true,\n  \"fallback\": \"default\"\n}",
            ),
            ("eval_ok/literal_union.reconf", "\"localhost\""),
            ("eval_ok/implicit_some.reconf", "{\n  \"port\": 8080\n}"),
            (
                "eval_ok/record_refinement.reconf",
                "[\n  {\n    \"ty\": \"localhost\",\n    \"addr\": null\n  },\n  {\n    \"ty\": \"fixed\",\n    \"addr\": \"127.0.0.1\"\n  }\n]",
            ),
            (
                "eval_ok/arithmetic_bool.reconf",
                "{\n  \"port\": 8080,\n  \"ok\": true,\n  \"msg\": \"reconf\"\n}",
            ),
        ];
        for (name, expected) in cases {
            let mut compiler = Compiler::new();
            let value = compiler
                .eval_file(&fixture_path(name))
                .unwrap_or_else(|err| {
                    panic!("{name} failed:\n{}", compiler.render(err));
                });
            assert_eq!(emit_json(&value, true).unwrap(), expected);
        }
    }
    ...
    fn check_negative_fixtures() {
        let cases = [
            ("refine_err/bad_port.reconf", "E_REFINE_004"),
            ("refine_err/bad_literal_union.reconf", "E_REFINE_002"),
            ("type_err/unknown_field.reconf", "E_RECORD_004"),
            ("type_err/missing_field.reconf", "E_RECORD_005"),
            ("type_err/none_without_expected_type.reconf", "E_TYPE_006"),
            ("type_err/duplicate_field.reconf", "E_RECORD_003"),
            ("type_err/type_mismatch.reconf", "E_TYPE_017"),
            ("module_err/unexported_main.reconf", "E_MODULE_005"),
            ("module_err/cycle_a.reconf", "E_MODULE_002"),
            ("parse_err/unterminated_string.reconf", "E_PARSE_008"),
            ("output_err/function_output.reconf", "E_OUTPUT_001"),
        ];
        for (name, expected_code) in cases {
            let mut compiler = Compiler::new();
            let err = compiler
                .check_file(&fixture_path(name))
                .expect_err(&format!("{name} unexpectedly passed"));
