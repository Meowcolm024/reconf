...
use reconf::resolve::resolved::{ResolvedModuleBody, ResolvedValueBindings};
use reconf::typeck::CoreModuleElaborator;
use reconf::typeck::CoreValueTypeContext;
...
fn module_elaborator_synthesizes_unannotated_literal_output() {
    ...
    assert!(matches!(
        elaborated.output,
        Some(ElaboratedExpr::Checked(reconf::core::TypedCoreExpr {
            expr: CoreExpr::Int(1),
            ty: CoreType::Int
        }))
    ));
...
fn module_elaborator_rejects_bare_none_without_expected_type() {
    ...
    assert_eq!(
        error.code(),
        reconf::error::ErrorCode::TypeNoneNeedsExpected
    );
}
...
fn module_elaborator_synthesizes_imported_value_references() {
    ...
    let elaborated =
        CoreModuleElaborator::with_context(&types, &values, reconf::typeck::CoreElaborator::new())
            .elaborate_resolved_module(decls, output)
            .unwrap();
...
fn module_elaborator_resolves_expression_locals_to_local_refs() {
    ...
    let Some(ElaboratedExpr::Checked(reconf::core::TypedCoreExpr {
