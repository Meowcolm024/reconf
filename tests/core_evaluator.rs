...
use reconf::core::{CoreExpr, CoreType, CoreTypeEnv, EmptyCoreTypeContext, GlobalRef, LocalRef};
use reconf::eval::core::{
    CoreEvaluator, PreparedCoreNormalizer, RuntimeValueApplicator, ValueApplicator,
};
use reconf::eval::{Env, Value};
use reconf::syntax::surface::Type;
...
fn prepared_core_normalizer_evaluates_typed_core_without_elaboration() {
    let env = Env::empty();
    let types = EmptyCoreTypeContext;
    let typed = reconf::core::TypedCoreExpr {
...
fn explicit_checked_path_expands_aliases_through_type_context() {
    ...
    aliases.insert(
        ...
        Type::Refinement {
            ...
            pred: Box::new(reconf::syntax::surface::Expr::Binary(
                ">".to_string(),
                Box::new(reconf::syntax::surface::Expr::Var("x".to_string())),
                Box::new(reconf::syntax::surface::Expr::Int(1024)),
            )),
        },
    );
    ...
    for (name, ty) in aliases {
        types.define(
            name,
            reconf::lower::SurfaceToCoreLowerer::new().lower_type(ty),
        );
    }
    let env = Env::empty();
...
struct TestCheckedNormalizer<'a> {
    env: Env,
    types: &'a dyn reconf::core::CoreTypeContext,
}

impl<'a> TestCheckedNormalizer<'a> {
    fn new(env: Env, types: &'a dyn reconf::core::CoreTypeContext) -> Self {
        Self { env, types }
    }

    fn synthesize(&self, expr: CoreExpr) -> reconf::Result<Value> {
        let expr = CoreElaborator::new().prepare_expr(expr)?;
        PreparedCoreNormalizer::new(self.env.clone(), self.types).synthesize(expr)
    }

    fn check(&self, expr: CoreExpr, ty: CoreType) -> reconf::Result<Value> {
