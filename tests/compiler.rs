use reconf::compiler::output::OutputValidator;
use reconf::compiler::session::CompilerSession;
use reconf::compiler::{CompileInput, Compiler, CompilerOptions, SourceInput};
...
use reconf::eval::builtins::NativeFunction;
use reconf::resolve::resolved::{ResolvedExport, ResolvedModuleBody};
use reconf::source::{LoadedSource, MemorySourceProvider, SourceProvider};
...
fn module_loader_records_loaded_modules_in_resolved_program() {
    ...
    let mut loader = reconf::compiler::loader::ModuleLoader::with_sources(
        sources,
        reconf::compiler::loader::ContextualModuleCompiler::with_prelude(),
    );
...
fn module_loader_frontend_errors_keep_import_source_labels() {
    ...
    assert_eq!(
        error.code(),
        reconf::error::ErrorCode::ParseEmptyInterpolation
    );
...
impl SourceProvider for CountingSourceProvider {
    fn load_source(&mut self, path: &Path) -> reconf::Result<LoadedSource> {
...
fn compiler_session_does_not_commit_failed_declarations() {
    ...
    assert_eq!(error.code(), reconf::error::ErrorCode::TypeUnknown);
...
fn empty_module_compiler_does_not_inject_prelude_context() {
    let mut loader = reconf::compiler::loader::ModuleLoader::with_sources(
        ...
        reconf::compiler::loader::EvaluatingModuleCompiler,
    );
...
fn output_validation_happens_when_requesting_data_output() {
    ...
    assert_eq!(error.code(), reconf::error::ErrorCode::OutputFunction);
}
