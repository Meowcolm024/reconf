...
fn eval_emits_json() {
    ...
    let file = dir.join("config.reconf_cli");
    fs::write(
        &file,
        r#"
...
fn check_rejects_bad_refinement_with_miette_report() {
    let dir = tempfile_dir();
    let file = dir.join("bad.reconf_cli");
    fs::write(
        &file,
        r#"
        type Port = { x : Int | x > 1024 && x < 65535 };
...
fn check_reports_imported_file_error_locations() {
    let dir = tempfile_dir();
    let main = dir.join("main.reconf_cli");
    let lib = dir.join("lib.reconf_cli");
    fs::write(&main, r#"import "./lib.reconf_cli": value; value"#).unwrap();
    fs::write(
        &lib,
        r#"
    ...
    assert!(!output.status.success());
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("lib.reconf_cli"), "{stderr}");
...
fn check_does_not_reject_function_output() {
    let dir = tempfile_dir();
    let file = dir.join("function_output.reconf_cli");
    fs::write(
        &file,
        r#"
        let hello = (g : Bool) =>
